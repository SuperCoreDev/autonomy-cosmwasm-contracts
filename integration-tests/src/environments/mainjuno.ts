...
import { setupCommon } from "../processes/setup/juno/mainnet/common";
import { setupStation } from "../processes/setup/juno/mainnet/station";
import { setupWrapperJunoswap } from "../processes/setup/juno/mainnet/wrapperJunoswap";

import { testExecuteStation } from "../processes/tests/juno/mainnet/station";
import { testExecuteWrapperJunoswap } from "../processes/tests/juno/mainnet/wrapperJunoswap";
...
async function initializeCommon() {
    wallet1 = await DirectSecp256k1HdWallet.fromMnemonic(mainnet.mnemonicKeys.wallet1, { prefix: "juno" });
    wallet2 = await DirectSecp256k1HdWallet.fromMnemonic(mainnet.mnemonicKeys.wallet2, { prefix: "juno" });
    wallet3 = await DirectSecp256k1HdWallet.fromMnemonic(mainnet.mnemonicKeys.wallet3, { prefix: "juno" });

    junod = await SigningCosmWasmClient.connectWithSigner(mainnet.networkInfo.url, wallet1, { gasPrice: GasPrice.fromString("0.025ujuno")});
...
async function initializeWrapperJunoSwap() {
    ...
    tcwUjunoSwap = mainnet.contracts.tcwUjunoSwap;
    wrapperJunoswap = mainnet.contracts.wrapperJunoSwap;
