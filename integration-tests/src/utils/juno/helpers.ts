...
export async function storeCode(
  ...
  const result = await junod.upload(account.address, code, "auto", );
...
export async function instantiateContract(
  ...
  const [account] = await deployer.getAccounts();
  const result = await junod.instantiate(account.address, codeId, instantiateMsg, "instantiate", "auto", { admin: account.address });
  return result;
}
...
export async function migrateContract(
  ...
  const result = await junod.migrate(account.address, contract, new_code_id, migrateMsg, "auto");
  return result;
}
