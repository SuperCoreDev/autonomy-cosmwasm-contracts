...
export async function testExecuteStation(
  ...
  //   counter,
  // );
  // await testFundsRouterDepositNativeSuccess(junod, wallet3, fundsRouter, "100", testnet.addresses.wallet3);
  // await testFundsRouterWithdrawNativeSuccess(junod, wallet3, fundsRouter, "100", testnet.addresses.wallet3);
  ...
  // await testTimeConditionsGetLastExecTime(junod, timeConditions, testnet.addresses.wallet1, 10);
  // await testTimeConditionsGetState(junod, timeConditions);

  // let start_time = Math.floor(Date.now() / 1000) - 60;
  // await testTimeConditionsSaveExecTime(junod, wallet3, timeConditions, testnet.addresses.wallet1, 1, start_time, 3);
...
async function testFundsRouterForwardCallsFails(
  ...
  let wallet3_client = await SigningCosmWasmClient.connectWithSigner(
    testnet.networkInfo.url, 
    wallet3, 
    {gasPrice: GasPrice.fromString("0.1ujunox")},
  );

  await expect(
    wallet3_client.execute(testnet.addresses.wallet3, fundsRouter, {
      forward_calls: {
        user: testnet.addresses.wallet1,
        fee_amount: fee_amount,
        fcn_data: [],
      },
    }, "auto", undefined, [coin(ujunox_amount, "ujunox")])
  ...
  await expect(
    wallet2_client.execute(testnet.addresses.wallet2, fundsRouter, {
      forward_calls: {
        user: testnet.addresses.wallet1,
        fee_amount: fee_amount,
        fcn_data: [],
...
// -----------------------------------------------
//  TEST: Wallet3 successfully "deposit" "ujunox" token to "FundsRouter"
//  
//  SCENARIO: 
//    Wallet3 deposits 100 "ujunox" for himself
// ------------------------------------------------
async function testFundsRouterDepositNativeSuccess(
 ...
 let wallet3_client = await SigningCosmWasmClient.connectWithSigner(
  ...
  expect(
    ...
    == parseInt(beforeBalanceQuery.balance) + parseInt(ujunox_amount)
  ).to.be.ok;
...
//  
//  SCENARIO: 
//    Wallet3 withdraws 100 "ujunox" for himself
// ------------------------------------------------
async function testFundsRouterWithdrawNativeSuccess(
 ...
 let wallet3_client = await SigningCosmWasmClient.connectWithSigner(
  ...
  const result = await wallet3_client.execute(
    ...
    fundsRouter, 
    {
      withdraw_fund: { recipient: recipient, amount: ujunox_amount },
    }, 
    "auto", 
    undefined, 
    [],
...
async function testTimeConditionsGetState(
  ...
  expect(res.router_user_veri_forwarder == testnet.addresses.wallet3).to.be.ok;
  console.log(chalk.green(" Passed!"));
}
...
async function testTimeConditionsSaveExecTime(
  ...
  await wallet3_client.execute(testnet.addresses.wallet3, timeConditions, {
    every_time_period: {
      user: user,
      call_id: id,
