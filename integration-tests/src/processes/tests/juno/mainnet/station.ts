...
export async function testExecuteStation(
  ...
  //   counter,
  // );
  // await testFundsRouterDepositNativeSuccess(junod, wallet3, fundsRouter, "100", mainnet.addresses.wallet3);
  // await testFundsRouterWithdrawNativeSuccess(junod, wallet3, fundsRouter, "100", mainnet.addresses.wallet3);
  ...
  // await testTimeConditionsGetLastExecTime(junod, timeConditions, mainnet.addresses.wallet1, 10);
  // await testTimeConditionsGetState(junod, timeConditions);

  // let start_time = Math.floor(Date.now() / 1000) - 60;
  // await testTimeConditionsSaveExecTime(junod, wallet3, timeConditions, mainnet.addresses.wallet1, 1, start_time, 3);
...
async function testFundsRouterForwardCallsFails(
  ...
  await expect(
    wallet3_client.execute(mainnet.addresses.wallet3, fundsRouter, {
      forward_calls: {
        user: mainnet.addresses.wallet1,
        fee_amount: fee_amount,
        fcn_data: [],
      },
    }, "auto", undefined, [coin(ujuno_amount, "ujuno")])
  ...
  await expect(
    wallet2_client.execute(mainnet.addresses.wallet2, fundsRouter, {
      forward_calls: {
        user: mainnet.addresses.wallet1,
        fee_amount: fee_amount,
        fcn_data: [],
...
// -----------------------------------------------
//  TEST: Wallet3 successfully "deposit" "ujuno" token to "FundsRouter"
//  
//  SCENARIO: 
//    Wallet3 deposits 100 "ujuno" for himself
// ------------------------------------------------
async function testFundsRouterDepositNativeSuccess(
 ...
 let wallet3_client = await SigningCosmWasmClient.connectWithSigner(
  ...
  expect(
    ...
    == parseInt(beforeBalanceQuery.balance) + parseInt(ujuno_amount)
  ).to.be.ok;
...
//  
//  SCENARIO: 
//    Wallet3 withdraws 100 "ujuno" for himself
// ------------------------------------------------
async function testFundsRouterWithdrawNativeSuccess(
 ...
 let wallet3_client = await SigningCosmWasmClient.connectWithSigner(
  ...
  const result = await wallet3_client.execute(
    ...
    fundsRouter, 
    {
      withdraw_fund: { recipient: recipient, amount: ujuno_amount },
    }, 
    "auto", 
    undefined, 
    [],
...
async function testTimeConditionsGetState(
  ...
  expect(res.router_user_veri_forwarder == mainnet.addresses.wallet3).to.be.ok;
  console.log(chalk.green(" Passed!"));
}
...
async function testTimeConditionsSaveExecTime(
  ...
  await wallet3_client.execute(mainnet.addresses.wallet3, timeConditions, {
    every_time_period: {
      user: user,
      call_id: id,
